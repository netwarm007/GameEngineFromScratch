...
void D3d12GraphicsManager::EndFrame(const Frame& frame) {
    ...
    if (SUCCEEDED(hr = m_pCommandList[m_nFrameIndex]->Close())) {
        ...
        if (FAILED(
                hr = m_pCommandQueue->Signal(m_pFence[m_nFrameIndex], fence))) {
            assert(0);
        }
    }
...
void D3d12GraphicsManager::DrawBatch(const Frame& frame) {
    for (const auto& pDbc : frame.batchContexts) {
        ...
        for (uint32_t i = 0; i < dbc.property_count; i++) {
            m_pCommandList[frame.frameIndex]->IASetVertexBuffers(
                i, 1, &m_VertexBufferView[dbc.property_offset + i]);
        }

        // select which index buffer to use
        m_pCommandList[frame.frameIndex]->IASetIndexBuffer(
            &m_IndexBufferView[dbc.index_offset]);

        // set primitive topology
        m_pCommandList[frame.frameIndex]->IASetPrimitiveTopology(
            D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

        // PerFrame CBV (b11)
        m_pCommandList[frame.frameIndex]->SetGraphicsRoot32BitConstants(
            1, 16, dbc.modelMatrix, 0);

        ID3D12DescriptorHeap* ppHeaps[] = {dbc.pCbvSrvUavHeap,
                                           m_pSamplerHeap[frame.frameIndex]};
        m_pCommandList[m_nFrameIndex]->SetDescriptorHeaps(
        ...
        m_pCommandList[frame.frameIndex]->SetGraphicsRootDescriptorTable(
...
void D3d12GraphicsManager::SetPipelineState(
    ...
    if (pipelineState) {
        ...
        m_pCommandList[m_nFrameIndex]->SetGraphicsRootSignature(
            pState->rootSignature);

        switch (pState->pipelineType) {
            case PIPELINE_TYPE::GRAPHIC: {
                // Per Frame CBV (b10)
                m_pCommandList[frame.frameIndex]
...
void D3d12GraphicsManager::SetLightInfo(const LightInfo& lightInfo) {
    memcpy(m_pLightInfoBegin[m_nFrameIndex], &lightInfo, sizeof(LightInfo));
}
