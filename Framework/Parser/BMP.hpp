                ...
                if (img.bitcount < 24) {
                    ...
                    for (int32_t y = img.Height - 1; y >= 0; y--) {
                        for (uint32_t x = 0; x < img.Width; x++) {
                            auto dst = reinterpret_cast<R8G8B8A8Unorm*>(reinterpret_cast<uint8_t*>(img.data) + (ptrdiff_t)img.pitch * ((ptrdiff_t)img.Height - y - 1) + (ptrdiff_t)x * byte_count);
                            auto src = reinterpret_cast<const R8G8B8A8Unorm*>(pSourceData + (ptrdiff_t)img.pitch * y + (ptrdiff_t)x * byte_count);
