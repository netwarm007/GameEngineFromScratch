...
v2p VSMain(a2v input) {
	...
	output.Position = mul(mul(mul(float4(input.Position.xyz, 1), m_worldMatrix), m_viewMatrix), m_projectionMatrix);
	float3 vN = (mul(mul(float4(input.Normal, 0), m_worldMatrix), m_viewMatrix)).xyz;
	output.vPosInView = (mul(mul(float4(input.Position.xyz, 1), m_worldMatrix), m_viewMatrix)).xyz;
