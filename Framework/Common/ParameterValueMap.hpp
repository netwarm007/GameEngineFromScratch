...
namespace My {
    ...
    struct ParameterValueMap
    {
        ...
        explicit ParameterValueMap(const T value) : Value(value), ValueMap(nullptr) {};
        explicit ParameterValueMap(std::shared_ptr<SceneObjectTexture>  value) : ValueMap(std::move(value)) {};
