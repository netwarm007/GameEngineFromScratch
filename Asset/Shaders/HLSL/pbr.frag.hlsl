...
float4 pbr_frag_main(pbr_vert_output _entryPointOutput) : SV_Target
{
    ...
    float meta = normalMap.Sample(samp0, texCoords).r; 
    float rough = normalMap.Sample(samp0, texCoords).g; 
    /*
    float3 tangent_normal.xy = float2(diffuseMap.Sample(samp0, texCoords).a, normalMap.Sample(samp0, texCoords).a);
    ...
    */
    ...
    {
        // ambient diffuse
        float ambientOcc = normalMap.Sample(samp0, texCoords).b;
