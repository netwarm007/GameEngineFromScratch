    ...
    void ConvertOddlStructureToSceneNode(
        ...
        switch (structure.GetStructureType()) {
            ...
            case OGEX::kStructureBoneNode: {
                auto _node = std::make_shared<SceneBoneNode>(
                    structure.GetStructureName());
            ...
            case OGEX::kStructureRotation: {
                ...
                if (kind == "x") {
                    rotation = std::make_shared<SceneObjectRotation>(
                        'x', data[0], object_flag);
                } else if (kind == "y") {
                    rotation = std::make_shared<SceneObjectRotation>(
                        'y', data[0], object_flag);
                } else if (kind == "z") {
                    rotation = std::make_shared<SceneObjectRotation>(
                        'z', data[0], object_flag);
                } else if (kind == "axis") {
                    rotation = std::make_shared<SceneObjectRotation>(
                ...
                while (_sub_structure) {
                    ...
                    switch (_sub_structure->GetStructureType()) {
                        ...
                        case OGEX::kStructureAtten: {
                            auto atten =
                                dynamic_cast<const OGEX::AttenStructure*>(
                                    _sub_structure);
                            AttenCurve curve;
                ...
                while (extension) {
                    ...
                    if (_appid == "MyGameEngine") {
                        ...
                        if (_type == "area_light") {
                            ...
                            auto _light =
                                dynamic_pointer_cast<SceneObjectAreaLight>(
                                    light);
                                ...
                                switch (value_array_size) {
                                    ...
                                    case 4: {
                                        value_curve = std::make_shared<
                                            ...
                                            value_key_data_count);
                                        type =
                                            SceneObjectTrackType::kQuoternion;
                                    } break;
                                    case 16: {
                                        value_curve = std::make_shared<
