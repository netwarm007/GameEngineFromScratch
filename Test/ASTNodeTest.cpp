...
int main(int argc, char* argv[])
{
    ...
    auto enum_  = std::make_shared<ASTNodeEnum<decltype(enum_items)>> ( "Enum", std::move(enum_items) );

    ASTList<ASTPair<std::string, std::string>> field_list { {"field1","int"}, {"field2","double"}, {"field3","string"} };
    auto struct_= std::make_shared<ASTNodeStruct<decltype(field_list)>>( "Struct", std::move(field_list) );

    ASTList<ASTPair<std::string, std::string>> record_list { {"field1","uuid"}, {"field2","Texture"}, {"field3","Shader"} };
    auto table  = std::make_shared<ASTNodeTable<decltype(record_list)>>( "Table", std::move(record_list) );
