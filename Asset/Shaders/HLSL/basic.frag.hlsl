...
float3 apply_light(const Light light, vert_output input) {
    ...
    float3 L;
    float3 light_dir = normalize((viewMatrix * light.lightDirection).xyz);
    ...
    {
        L = (viewMatrix * light.lightPosition).xyz - input.v.xyz;
    }
...
{
    float3 N = normalize(input.normal.xyz);
    float3 right = normalize((viewMatrix * float4(1.0f, 0.0f, 0.0f, 0.0f)).xyz);
    float3 pnormal = normalize((viewMatrix * light.lightDirection).xyz);
    float3 ppos = (viewMatrix * light.lightPosition).xyz;
