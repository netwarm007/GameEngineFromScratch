...
vec3 apply_areaLight(Light light)
{
    ...
    vec3 right = normalize((_89.viewMatrix * vec4(1.0, 0.0, 0.0, 0.0)).xyz);
    vec3 pnormal = normalize((_89.viewMatrix * light.lightDirection).xyz);
    vec3 ppos = (_89.viewMatrix * light.lightPosition).xyz;
...
{
    vec3 N = normalize(normal.xyz);
    vec3 light_dir = normalize((_89.viewMatrix * light.lightDirection).xyz);
    vec3 L;
